...
export class HSMLMultiParser {
    ...
    private parseShapeForce(): ShapeForceNode {
        const start = this.current;
        this.consume(TokenType.FORCE, "Expected 'force' keyword");
        
        const forceType = this.consume(TokenType.IDENTIFIER, "Expected force type").value as 'gravity' | 'elastic' | 'viscous' | 'electromagnetic';
    ...
    private parseShapeConstraint(): ShapeConstraintNode {
        const start = this.current;
        this.consume(TokenType.CONSTRAINT, "Expected 'constraint' keyword");
        
        const constraintType = this.consume(TokenType.IDENTIFIER, "Expected constraint type").value as 'spherical_surface' | 'radial_range' | 'angular_cone';
    ...
    private parseShapeEvent(): ShapeEventNode {
        const start = this.current;
    ...
    private parseStyleBotAgent(): StyleBotAgentNode {
        const start = this.current;
        ...
        const render: StyleBotRenderNode[] = [];
        const optimize: ExpressionNode[] = [];
    ...
    private parseStyleBotRender(): StyleBotRenderNode {
        const start = this.current;
    ...
    private parseEquality(): ExpressionNodeType {
        ...
        while (this.match(TokenType.EQUALS, TokenType.NOT_EQUALS)) {
            ...
            expr = {
                ...
                language: this.language
            } as ExpressionNodeType;
        }
        
        return expr;
    }
    
    private parseComparison(): ExpressionNodeType {
    ...
    private parseTerm(): ExpressionNodeType {
    ...
    private parseFactor(): ExpressionNodeType {
    ...
    private parseUnary(): ExpressionNodeType {
    ...
    private parseCall(): ExpressionNodeType {
        let expr: ExpressionNodeType = this.parsePrimary();
        ...
        return expr as ExpressionNodeType;
    }
    
    private finishCall(callee: ExpressionNodeType): CallExpressionNode {
    ...
    private parsePrimary(): LiteralNode | IdentifierNode {
    ...
    private parseSphericalPrimary(): SphericalCoordinateNode | SolidAngleNode | MatterStateNode {
