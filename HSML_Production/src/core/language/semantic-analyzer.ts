...
export class HSMLSemanticAnalyzer {
    ...
    analyze(ast: ProgramNode): { errors: SemanticError[]; warnings: SemanticError[] } {
        ...
        this.validatePhysicsConsistency();
    ...
    private analyzeHSMLElement(element: HSMLElementNode): void {
        ...
        const elementSymbol: ElementSymbol = {
            id: element.attributes.find(attr => attr.name === 'id')?.value?.expression?.name || this.generateElementId(),
        ...
        if (positionAttr) {
            this.validateSphericalCoordinate(positionAttr.value);
        }
    ...
    private checkUndefinedReferences(): void {
        ...
        for (const element of this.symbolTable.elements.values()) {
        ...
        for (const bot of this.symbolTable.bots.values()) {
            for (const agent of bot.agents) {
    ...
    private validatePhysicsConsistency(): void {}
} 